...
module.exports = {
  ...
  deploy: {
    production: {
      ...
      "post-deploy": `source $HOME/.nvm/nvm.sh; nvm use v${NODE_VERSION}; npm install; export $(grep -v '^#' /home/admin/plumb-all-slack-integration/.env | grep '=' | xargs) && npx prisma generate; pm2 startOrRestart ecosystem.config.cjs --env production`
    },
  },
};
