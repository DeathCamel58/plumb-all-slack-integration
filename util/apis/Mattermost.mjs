 ...
 */
async function sendMessage(message, username, channelName = mattermostCallChannelName) {
    ...
    try {
        ...
        if (!channelId) {
            console.error(`Couldn't determine channel ID for ${channelName}. Found channels are:\n${JSON.stringify(channels)}`)
        }
        ...
        for (let i = 0; i < needToUnfurl.quotes.length; i++) {
            if (needToUnfurl.quotes[i] !== null) {
                ...
                data.push({
                    ...
                    totalToPrint: needToUnfurl.quotes[i].amounts.total.toLocaleString('en-US', {style: 'currency', currency: 'USD'}),
        ...
        for (let i = 0; i < needToUnfurl.jobs.length; i++) {
            if (needToUnfurl.jobs[i] !== null) {
                ...
                data.push({
                    ...
                    totalToPrint: needToUnfurl.jobs[i].amounts.total.toLocaleString('en-US', {style: 'currency', currency: 'USD'}),
        ...
        for (let i = 0; i < needToUnfurl.invoices.length; i++) {
            if (needToUnfurl.invoices[i] !== null) {
                ...
                data.push({
                    ...
                    totalToPrint: needToUnfurl.invoices[i].amounts.total.toLocaleString('en-US', {style: 'currency', currency: 'USD'}),
...
wsClient.addMessageListener((msg) => {
    if (process.env.DEBUG === "TRUE") {
        console.log(`Message received from WebSocket: ${msg.event}\n${JSON.stringify(msg.data)}`);
    }
    if (msg.event === 'posted') {
